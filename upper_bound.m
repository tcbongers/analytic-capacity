...
function minimum_integral = upper_bound(centers, radius, poles)
...
%x = (-b/A)';
%minimum_integral = 0.5 * (x')*(A*x) + b*x + c;

A_inv = inv(A);
minimum_integral = c - 0.5*b*A_inv*b';

D_inv = zeros(length(A));
for k = 1:length(D_inv)
    D_inv(k, k) = 1 / abs(b(k))^2 / (2*length(D_inv));
end
